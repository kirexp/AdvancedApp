...
namespace Common
{
    public class SMSC
    {
        ...
        public string[] send_sms(string phones, string message, int translit = 0, string time = "", int id = 0, int format = 0, string sender = "", string query = "", string[] files = null)
        {
            ...
            string[] m = this._smsc_send_cmd("send", "cost=3&phones=" + this._urlencode(phones)
                            + "&mes=" + this._urlencode(message) + "&id=" + id.ToString() + "&translit=" + translit.ToString()
                            + (format > 0 ? "&" + formats[format - 1] : "") + (sender != "" ? "&sender=" + this._urlencode(sender) : "")
                            + (time != "" ? "&time=" + this._urlencode(time) : "") + (query != "" ? "&" + query : ""), files);
            ...
            if (SMSC_DEBUG)
            {
                if (Convert.ToInt32(m[1]) > 0)
                    this._print_debug("Сообщение отправлено успешно. ID: " + m[0] + ", всего SMS: " + m[1] + ", стоимость: " + m[2] + ", баланс: " + m[3]);
                else
                    this._print_debug("Ошибка №" + m[1].Substring(1, 1) + (m[0] != "0" ? ", ID: " + m[0] : ""));
            }
        ...
        public string[] get_sms_cost(string phones, string message, int translit = 0, int format = 0, string sender = "", string query = "")
        {
            ...
            string[] m = this._smsc_send_cmd("send", "cost=1&phones=" + this._urlencode(phones)
                            + "&mes=" + this._urlencode(message) + translit.ToString() + (format > 0 ? "&" + formats[format - 1] : "")
                            + (sender != "" ? "&sender=" + this._urlencode(sender) : "") + (query != "" ? "&query" : ""));
            ...
            {
                if (Convert.ToInt32(m[1]) > 0)
                    this._print_debug("Стоимость рассылки: " + m[0] + ". Всего SMS: " + m[1]);
                else
                    this._print_debug("Ошибка №" + m[1].Substring(1, 1));
            }
        ...
        {
            string[] m = this._smsc_send_cmd("status", "phone=" + this._urlencode(phone) + "&id=" + this._urlencode(id) + "&all=" + all.ToString());
            ...
            if (id.IndexOf(',') == -1)
            {
                if (SMSC_DEBUG)
                {
                    if (m[1] != "" && Convert.ToInt32(m[1]) >= 0)
                    {
                        ...
                        this._print_debug("Статус SMS = " + m[0] + (timestamp > 0 ? ", время изменения статуса - " + date.ToLocalTime() : ""));
                    }
                    else
                        this._print_debug("Ошибка №" + m[1].Substring(1, 1));
                }
            ...
            else
            {
                ...
                Array.Resize(ref this.D2Res, 0);
                Array.Resize(ref this.D2Res, m.Length);

                for (int i = 0; i < this.D2Res.Length; i++)
                    this.D2Res[i] = m[i].Split(',');
        ...
        {
            string[] m = this._smsc_send_cmd("balance", ""); // (balance) или (0, -error)

            if (SMSC_DEBUG)
            {
                if (m.Length == 1)
                    this._print_debug("Сумма на счете: " + m[0]);
                else
                    this._print_debug("Ошибка №" + m[1].Substring(1, 1));
            }
        ...
        {
            arg = "login=" + this._urlencode(SMSC_LOGIN) + "&psw=" + this._urlencode(SMSC_PASSWORD) + "&fmt=1&charset=" + SMSC_CHARSET + "&" + arg;

            string url = (SMSC_HTTPS ? "https" : "http") + "://smsc.kz/sys/" + cmd + ".php" + (this.SMSC_POST ? "" : "?" + arg);
                ...
                if (this.SMSC_POST)
                {
                    ...
                    else
                    {
                        ...
                        for (int pcnt = 0; pcnt < par.Length + fl; pcnt++)
                        {
                            ...
                            output = this._concatb(output, postHeaderBytes);
                            ...
                            if (pof)
                            {
                                ...
                                while ((bytesRead = fileStream.Read(buffer, 0, buffer.Length)) != 0)
                                {
                                    ...
                                    output = this._concatb(output, tbuf);
                                }
                            }
                            else
                            {
                                byte[] vl = Encoding.UTF8.GetBytes(nv[1]);
                                output = this._concatb(output, vl);
                            }

                            output = this._concatb(output, Encoding.UTF8.GetBytes("\r\n"));
                        }
                        output = this._concatb(output, boundaryBytes);
            ...
            {
                if (SMSC_DEBUG)
                    this._print_debug("Ошибка чтения адреса: " + url);
