...
{
    public class Http {
        private readonly string BaseUri = "http://95.59.125.132:1133";
        ...
        public async Task<TResponse> GetAsync<TResponse>(string uri) where TResponse : class {
            ...
            var res = await httpClient.GetAsync(this.BaseUri+"/"+uri);
        ...
        public async Task<TResponse> PostAsJson<TResponse>(string uri, object request, bool defaultJsonSettings = false)
        {
            ...
            var requestMessage = new HttpRequestMessage(HttpMethod.Post, new Uri(this.BaseUri + "/" + uri));
